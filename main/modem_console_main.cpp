...
#error Invalid serial connection to modem.
    ...
    {
        GenericCommandArgs() : cmd(STR1, nullptr, nullptr, "<command>", "AT command to send to the modem"),
    ...
    const struct StartApArgs
    {
        StartApArgs() : ssid(STR1, nullptr, nullptr, "<ssid>", "AP SSID name"),
                        password(STR0, "p", "password", "<password>", "AP password (optional)"),
                        channel(INT0, "c", "channel", "<channel>", "WiFi channel (default: 1)") {}
        CommandArgs ssid;
        CommandArgs password;
        CommandArgs channel;
    } start_ap_args;

    const ConsoleCommand StartAP("start_ap", "Start WiFi AP with PPPoS connection sharing", &start_ap_args, sizeof(start_ap_args), [&](ConsoleCommand *c)
                                 {
        auto ssid = c->get_string_of(&StartApArgs::ssid);
        auto password = c->get_string_of(&StartApArgs::password);
        auto channel = c->get_count_of(&StartApArgs::channel) ? c->get_int_of(&StartApArgs::channel) : 1;
        ...
        ESP_LOGI(TAG, "Initializing WiFi Access Point");
         ...
         assert(ap_netif);
         // Get DNS from PPP interface and set it for AP DHCP server
         esp_netif_dns_info_t dns;
         esp_err_t err = esp_netif_get_dns_info(esp_netif, ESP_NETIF_DNS_MAIN, &dns);
         if (err != ESP_OK) {
             ESP_LOGE(TAG, "Failed to get DNS info: %s", esp_err_to_name(err));
          return -1;  // Return an error or handle appropriately
         }
        ...
        wifi_config.ap.channel = static_cast<uint8_t>(channel);
        ...
        wifi_config.ap.ssid_len = strlen(ssid.c_str());
        if (!password.empty()) {
        ...
        if(esp_netif_is_netif_up(ap_netif) != true){
            ESP_LOGI(TAG, "Failed to start AP");
        } else {
            ESP_LOGI(TAG, "AP started successfully");
        }
        // Enable NAT
        ESP_LOGI(TAG, "Enabling NAT on interface %lu", _g_esp_netif_soft_ap_ip.ip.addr);
    ...
    {
        SetPsmCfgArgs() : periodic_tau(STR1, nullptr, nullptr, "<Requested_Periodic-TAU>", "T3412 Timer in 8-bit format"),
